...
function renderOperonHTML(nodeMeta) {
  ...
  const genes = [...nodeMeta.operon.genes].sort((a, b) => Number(a.start) - Number(b.start));
...
function renderScaleHTML(regionStart, regionEnd) {
  ...
  const ticks = fractions.map((fraction, i) => {
...
function positionTooltip(e) {
  ...
  left = clamp(left, window.scrollX + 8, window.scrollX + window.innerWidth - width - 8);
  top = clamp(top, window.scrollY + 8, window.scrollY + window.innerHeight - height - 8);
